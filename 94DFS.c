/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2018年08月17日 星期五 10时36分04秒
 ************************************************************************/
...
typedef struct {
    VertexType vexs[MAX_VERtEX_NUM];            //存储图中的顶点数据
    AdjMatrix arcs;                             //二维数组，记录顶点之间的关系
    int vexnum, arcnum;                         //记录图中的顶点数和弧（边） 数
}MGraph;
...
void CreateDN(MGraph *G){
    ...
    for (int i=0; i<G->arcnum; i++) {
        ...
        G->arcs[n][m].adj=1;
        G->arcs[m][n].adj=1;//无向图的二姐矩阵沿主对角线对称 
    }
}
...
void DFS(MGraph G, int v){
    ...
    //从该顶点的第一个边开始，一直到最后一个边，对处于另一端的顶点调用DFS函数
