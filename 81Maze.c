...
int main(){
        printf("***阿乾迷宫小项目--by-Leecheer--***\n");
        char maze[ROWS][COLS];
        int xStart, yStart, x, y;
        srand(time(0));//随机数，每次运行种下不同的种子，后序通过rand()函数获得随机数就不同通过一个嵌套循环，先将迷宫中各个地方地方设置为非通路('#'表示为墙，表示此处不可通过)
        ...
        printf("迷宫入口位置坐标为(%d,%d);出口坐标为(%d, %d);\n",xStart+1,yStart+1,x+1,y+1);
        printf("迷宫设置如下('#'表示墙，‘0’表示通路)");
        printMaze(maze);//输出一个初始化的迷宫
        //使用回溯法，通过不断进行尝试，试图找到一个通往出口的路
        mazeTraversal(maze,xStart,yStart,xStart,yStart,x,y); 
}
//迷宫整体布局为矩形，有四条边，在初始化迷宫的出口和入口时，随机选择不同的两条边作为设置出口和入口的边
void mazeGenerator(char maze[][COLS], int *xPtr, int *yPtr, int *exitx, int *exity){
        ...
        //确定入口位置，0代表选择的为左侧的边，1代表为上边，2代表为右侧的边，3代表为下边
        if(entry==0){
                *xPtr=1+rand()%(ROWS-2);
                *yPtr=0;
                maze[*xPtr][*yPtr]='0';
...
void mazeTraversal(char maze[ROWS-2][COLS], int row, int col, int entryRow, int entryCol, int exitrow, int exitcol){
        //由于从入口出进入，为了区分走过的通路和没走过的通路，将走过的通路设置为‘x’
        maze[row][col]='x';
        static bool judge=false;//设置一个判断变量，判断在入口是否有通路存在
        static int succ=0;//用于统计从入口到出口的可信通路的条数
        if (row == exitrow && col == exitcol){
                printf("成功走出迷宫，像是介个样子:\n");
                printMaze(maze);
                succ++;
                return;
        }
        ...
        if(validMove (maze,row+1, col)){
                judge=true;//证明起码有path存在，证明是否有可通往出口的路
...
//有效移动，即证明该位置处于整个迷宫的矩形范围内，且该位置为通路，不是墙，也从未走过
