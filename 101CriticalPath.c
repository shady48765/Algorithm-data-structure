/*************************************************************************
	> File Name: 101CriticalPath.c
	> Author: 
	> Mail: 
	> Created Time: 2018年08月26日 星期日 15时10分57秒
 ************************************************************************/
...
void CreateAOE(ALGraph **G){
    *G=(ALGraph*)malloc(sizeof(ALGraph));
    ...
    for (int i=0; i<(*G)->arcnum; i++) {
        ...
        p->nextarc=(*G)->vertices[locate].firstarc;//头结点插入
...
void FindInDegree(ALGraph G,int indegree[]){
    ...
    for (int i=0; i<G.vexnum; i++) {
        ArcNode *p=G.vertices[i].firstarc;//*firstarc为指向邻接点的指针
        while (p) {//
            indegree[p->adjvex]++;//
            p=p->nextarc;
        }
...
bool TopologicalOrder(ALGraph G){
    ...
    while (!StackEmpty(*S)) {
        ...
        for (ArcNode *p=G.vertices[index].firstarc; p ; p=p->nextarc) {
            ...
            if (ve[index]+p->dut>ve[k]) {//ve[k]=0,k=p->adjvex 多个邻接点中选择出来，最长的加到ve[k]中去。index为前一个k,push(S,k),pop出来的
                ve[k]=ve[index]+p->dut;
            }
        }
    }
...
void CriticalPath(ALGraph G){
    if (!TopologicalOrder(G)) {//false,无回路
        return ;//跳出
    }
    for (int i=0 ; i<G.vexnum ; i++) {
        vl[i]=ve[G.vexnum-1];//以上求出来的ve[k],k=G.vexnum-1
    }
    int j,k;
    while (!StackEmpty(*T)) {//while (false), *T->next不等于NULL，排序完成指向NULL,此时，跳出循环
        pop(T, &j);//出栈
        for (ArcNode* p=G.vertices[j].firstarc ; p ; p=p->nextarc) {//表示该节点j的邻接点k，且是每个邻接点k都要遍历。
            k=p->adjvex;
            //构建Vl数组，在初始化时，Vl数组中每个单元都是18，如果每个边的汇点-边的权值比源点值小，就保存更小的。
            if (vl[k]-p->dut<vl[j]) {//vl[j]初始为18， vl[k]初始为18， 所以，减去p->dut, 必然小于vl[j],条件满足后， 赋值vl[j]=18-p->dut[max]
                vl[j] = vl[k]-p->dut;
            }
        }
    }
