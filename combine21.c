...
void freeArr(component * array, int body);
...
//提取分配空间
int mallocArr(component * array) {
        // 若备用链表非空，则返回分配的结点小标，否则返回0（当分配最后一个结点时，该节点的游标值为0
        int i=array[0].cur;
        if (array[0].cur) {
                array[0].cur=array[i].cur;//把非空游标改成最后一个数据的0游标
                                            //这样备用链表就是空的了
        }
        return i;
}
...
int initArr(component * array){
        ...
        //声明一个变量，把它当做指针使，指向链表的最后一个结点，因为链表为空，所以和头结点重合
        int tempBody=body;
        for (int i=1;i<5;i++) {
                int j=mallocArr(array);//从备用链表拿出空闲的分量
                array[tempBody].cur=j;//将申请的空闲分量链接在链表的最后一个结点后面
                array[j].data='a'+i-1;//给新申请的分量的数据域初始化
                tempBody=j;//将指向链表最后一个结点
        }
