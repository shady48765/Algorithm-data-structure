...
zanInode pop(zanInode *park, int *parktop, int carnumber, zanInode *location, int *locationtop) {
        ...
        if (i==(*parktop)) {
                ...
                while ((*parktop)>i+1) {
                        (*parktop)--;
                        location[*location]=park[*parktop];
                        (*locationtop)++;
                }
                //通过以上的循环，可以上该车后的左右车辆全部移开，但是由于该车也要出栈，所以栈顶指针需要下移一个位置，当车进栈时，就直接覆盖掉了
                (*parktop)--;
                thecar=park[*parktop];
                //该车出栈后，还要将之前出站的所有车，再全部进栈
                while ((*locationtop)>0) {
                        (*loctiontop)--;
                        park[*parktop]=location[*location];
                        (*parktop)++;
                }
        }
        return thecar;
        ...
        while (scanf("%c",&order) ) {
                ...
                switch (order) {
                        ...
                        case 'D':
                                ...
                                //如果返回的车的车牌号为-1，表明在停车场中没有查找到该车
                                if (car.number!=-1) {
                                        //停留时间，等于出停车场的时间-进停车场的时间
                                        time=leaveTime-car.ariive_time;
                                        printf("该车停留的时间为：%d小时，应缴费为:%f元\n",time,time*1.5);
                                        //一旦有车离开停车场，则在便道中先等待的就可以进入，进入需设定车进入时间
                                        if (front!=rear) {
                                                car=accessroad[front];
                                                printf("在便道上第1的位置，车牌号为:%d的车进停车场的时间为：\n",car.number);
                                                scanf("%d", &car.arrive_time);
                                                park[parktop]=car;
                                                front++;
                                                parktop++;
                                        }else{
                                                printf("便道上没有等待车辆，停车场未满\n");
                                        }
                                }
                                break;
                            default:
