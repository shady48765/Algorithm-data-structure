...
RLSMatrix MultSMatrix(RLSMatrix A, RLSMatrix B, RLSMatrix C)
{
        ...
        else
        {
                ...
                {
                        //创建一个临时存储成绩结果的数组，且初始化为0， 遍历每次都需要清空
                        
                        int ctemp[MAXRC+1]={};
                        C.rpos[arow]=C.tu+1;
                        //根据函数，在三元组表中找到改行所有的非0元素的位置
                        int tp;
                        if(arow<=A.mu)
                                tp = A.rpos[arow+1];//获取矩阵A的下一行的第一个非零元素在data数组中位置
                        else
                                tp = A.tu + 1;// 若当前行是最后一行，则去最后一个元素+1
                        ...
                        {
                                brow =A.data[p].j;//取该非0元素的列数，便于去B中对应的做乘积的非0元素
                                int t;
                                // 判断如果对于A中非0元素，找到矩阵B中做乘法的哪一行中的所有的非0元素
                                if(brow < B.mu)
                                        t = B.rpos[brow + 1];
                                else
                                        t = B.tu+1;
                                ...
                                {
                                        //得到的乘积结果，每次和ctemp数组中对应未知的数组做加和运算
                                        ccol =B.data[q].j;
